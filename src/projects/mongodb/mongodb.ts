// Single source of truth for the mongodb navbar + project landing.
...
export type MongoDbChapter = string;

export const CHAPTERS: { id: string; label: string }[] = [];

export interface MongoDbTopic {
    slug: string; // URL segment -> /mongodb/<slug>
    name: string; // nav + card label
    chapter: string; // CHAPTERS id
    summary: string; // one-line blurb
}

export const TOPICS: MongoDbTopic[] = [];
