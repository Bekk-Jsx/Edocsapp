// Single source of truth for the couchdb navbar + project landing.
...
export type CouchDbChapter = string;

export const CHAPTERS: { id: string; label: string }[] = [];

export interface CouchDbTopic {
    slug: string; // URL segment -> /couchdb/<slug>
    name: string; // nav + card label
    chapter: string; // CHAPTERS id
    summary: string; // one-line blurb
}

export const TOPICS: CouchDbTopic[] = [];
