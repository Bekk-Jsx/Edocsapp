// Single source of truth for the technical-seo navbar + project landing.
...
export type TechnicalSeoChapter = string;

export const CHAPTERS: { id: string; label: string }[] = [];

export interface TechnicalSeoTopic {
    slug: string; // URL segment -> /technical-seo/<slug>
    name: string; // nav + card label
    chapter: string; // CHAPTERS id
    summary: string; // one-line blurb
}

export const TOPICS: TechnicalSeoTopic[] = [];
