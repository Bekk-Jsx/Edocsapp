// Global "jump to react vs next.js" control. Fixed bottom-right, rendered once
// in the root layout so it exists on every hook page. Pure anchor + CSS: the
// scroll is native (html { scroll-behavior: smooth }), the tooltip is a sibling
// span toggled on :hover / :focus-visible. No JS, so this stays a Server
// Component.
//
// Target: #react-vs-next-js. NOTE: the five Next-only hook pages (use-params,
// use-pathname, use-router, use-search-params, use-selected-layout-segment)
// title that footer "no react equivalent · next.js only", so the anchor is a
// no-op there — a missing target simply does nothing.
export default function FaqButton() {
    return (
        <a
            href="#react-vs-next-js"
            aria-label="Jump to React vs Next.js"
